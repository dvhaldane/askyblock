...
public class Island implements Cloneable {
    ...
    // Tile entities
    private Multiset<ItemType> tileEntityCount = HashMultiset.create();
    // Biome
    BiomeType biome;
    ...
    public Island(ASkyBlock plugin, String serial) {
        ...
        try {
            ...
            this.center = new Location<World>(world, x, y, z);
            ...
            if (!isSpawn) {
                ...
                if (split.length > 9) {
                    try {
                        biome = Sponge.getRegistry().getType(BiomeType.class, split[9]).get();
     ...
     */
    public boolean onIsland(Location<World> target) {
        if (world != null) {
            ...
            if (target.getExtent().equals(world) || (Settings.createNether && Settings.newNether && ASkyBlock.getNetherWorld() != null && target.getExtent().equals(ASkyBlock.getNetherWorld()))) {
     ...
     */
    public boolean inIslandSpace(Location<World> target) {
        if (target.getExtent().equals(ASkyBlock.getIslandWorld()) || target.getExtent().equals(ASkyBlock.getNetherWorld())) {
    ...
    public int getPopulation() {
        ...
        for (int x = getMinProtectedX() /16; x <= (getMinProtectedX() + getProtectionSize() - 1)/16; x++) {
            for (int z = getMinProtectedZ() /16; z <= (getMinProtectedZ() + getProtectionSize() - 1)/16; z++) {
                for (Entity entity : world.getChunk(x, 0, z).get().getEntities()) {
    ...
    public int getHopperCount() {
        ...
        for (int x = getMinProtectedX() /16; x <= (getMinProtectedX() + getProtectionSize() - 1)/16; x++) {
            for (int z = getMinProtectedZ() /16; z <= (getMinProtectedZ() + getProtectionSize() - 1)/16; z++) {
                for (TileEntity holder : world.getChunk(x, 0, z).get().getTileEntities()) {
                    if (holder instanceof Hopper && onIsland(((Hopper) holder).getLocation())) {
                        result++;
                    }
                }
            }  
     ...
     * a tile entity.
     */
    public int getTileEntityCount(TileEntity entity) {
        int result = 0;
        for (int x = getMinProtectedX() /16; x <= (getMinProtectedX() + getProtectionSize() - 1)/16; x++) {
            for (int z = getMinProtectedZ() /16; z <= (getMinProtectedZ() + getProtectionSize() - 1)/16; z++) {
                for (TileEntity holder : world.getChunk(x, 0, z).get().getTileEntities()) {
                    ...
                    if (onIsland(holder.getLocation())) {
                        if (holder.getType() == entity) {
                            result++;
                        } else if (entity.getType().equals(TileEntityTypes.COMPARATOR)) {
                            if (holder.getType().equals(TileEntityTypes.COMPARATOR)) {
                                result++;
                            }
                        } else if (entity.getType().equals(TileEntityTypes.FURNACE)) {
                            if (holder.getType().equals(TileEntityTypes.FURNACE)) {
                                result++;
                            }
                        } else if (entity.getType().equals(TileEntityTypes.BANNER)) {
                            if (holder.getType().equals(TileEntityTypes.BANNER)) {
                                result++;
                            }
                        } else if (entity.getType().equals(TileEntityTypes.SIGN)) {
                            if (holder.getType().equals(TileEntityTypes.SIGN)) {
                                result++;
                            }
                        }
                    }
                }
                for (Entity holder : world.getChunk(x, 0, z).get().getEntities()) {
                    //plugin.getLogger().info("DEBUG: entity: " + holder.getType());
                    if (holder.getType().toString().equals(entity.toString()) && onIsland(holder.getLocation())) {
                        result++;
                    }
                }
            }  
