...
public class Players {
	...
	private HashMap<Integer, Location<World>> homeLocations;
	...
	private HashMap<Location<World>, Date> kickedList;
	...
	public Players(final ASkyBlock aSkyBlock, final UUID uuid) {
		...
		// this.homeLocation<World> = null;
		this.homeLocations = new HashMap<Integer, Location<World>>();
		this.inTeam = false;
		...
		this.kickedList = new HashMap<Location<World>, Date>();
		this.locale = "";
	...
	public boolean hasIsland() {
		...
		if (hasIsland && islandLocation.isEmpty()) {
			hasIsland = false;
			plugin.getLogger().error(playerName + " apparently had an island, but the Location<World> is unknown.");
		}
		return hasIsland;
	}
	...
	public boolean inTeam() {
		...
		if (inTeam && teamIslandLocation.isEmpty()) {
			...
			if (teamLeader == null) {
				...
				inTeam = false;
				plugin.getLogger().error(playerName
						+ " was listed as in a team, but has no team island or team leader. Removing from team.");
			} else {
				...
				if (plugin.getPlayers().getMembers(teamLeader).contains(uuid)) {
					...
					if (plugin.getPlayers().getTeamIslandLocation(teamLeader) != null) {
						teamIslandLocation = Util
								.getStringLocation(plugin.getPlayers().getTeamIslandLocation(teamLeader));
						plugin.getLogger()
								.error(playerName + " was listed as in a team, but has no team island. Fixed.");
					}
				} else {
					inTeam = false;
					teamLeader = null;
	...
	/**
	 * Gets the home Location<World> by number.
	 * 
	 * @param number
	 * @return Location<World> of this home or null if not available
	 */
	public Location<World> getHomeLocation(int number) {
	...
	/**
	 * Provides a list of all home locations - used when searching for a safe
	 * spot to place someone
	 * 
	 * @return List of home locations
	 */
	public HashMap<Integer, Location<World>> getHomeLocations() {
		HashMap<Integer, Location<World>> result = new HashMap<Integer, Location<World>>();
	...
	/**
	 * @return the Location<World> of the player's island in Location<World>
	 *         form
	 */
	...
	public Location<World> getTeamIslandLocation() {
		// return teamIslandLoc.getLocation();
		if (teamIslandLocation == null || teamIslandLocation.isEmpty()) {
			return null;
		}
		Location<World> l = Util.getLocationString(teamIslandLocation);
		return l;
	}
	...
	/**
	 * Stores the home Location<World> of the player in a String format
	 * 
	 * @param l
	 *            a Bukkit location
	 */
	public void setHomeLocation(final Location<World> l) {
		setHomeLocation(l, 1);
	}

	/**
	 * Stores the numbered home Location<World> of the player. Numbering starts
	 * at 1.
	 * 
	 * @param location
	 * @param number
	 */
	public void setHomeLocation(final Location<World> location, int number) {
		if (location == null) {
			homeLocations.clear();
		} else {
			// Make the Location<World> x,y,z integer, but keep the yaw and
			// pitch
			homeLocations.put(number, new Location<World>(location.getExtent(), location.getBlockX(),
	...
	/**
	 * Records the player's island Location<World> in a string form
	 * 
	 * @param l
	 *            a Bukkit Location
	 */
	public void setIslandLocation(final Location<World> l) {
	 ...
	 * @param l
	 *            - the Bukkit Location<World> of the team's island (converted
	 *            to a String in this function)
	 */
	public void setJoinTeam(final UUID leader, final Location<World> l) {
		if (inTeam) {
			Sponge.getEventManager().post(new TeamLeaveEvent(uuid, teamLeader));
		}
		...
		Sponge.getEventManager().post(new TeamJoinEvent(uuid, leader));
	}
	...
	/**
	 * @param l
	 *            a Bukkit Location<World> of the team island
	 */
	public void setTeamIslandLocation(final Location<World> l) {
		teamIslandLocation = Util.getStringLocation(l);
	}
	 ...
	 */
	public long getInviteCoolDownTime(Location<World> location) {
		// Check the hashmap
		if (location != null && kickedList.containsKey(location)) {
			// plugin.getLogger().info("DEBUG: Location<World> is known");
			// The Location<World> is in the list
			// Check the date/time
	...
	/**
	 * Starts the invite cooldown timer for location. Location<World> should be
	 * the center of an island.
	 * 
	 * @param location
	 */
	public void startInviteCoolDownTimer(Location<World> location) {
		if (location != null) {
	...
	/**
	 * @param startIslandRating
	 *            the startIslandRating to set
	 */
	...
	/**
	 * @return a list of challenges this player has completed Used by the reset
	 *         admin command
	 */
	...
	/**
	 * @return a list of challenges this player has not completed Used by the
	 *         complete admin command
	 */
