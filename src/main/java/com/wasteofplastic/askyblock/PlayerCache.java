...
public class PlayerCache {
     ...
     */
    /**
     * Returns Location<World> of player's island from cache if available
     * 
     * @param playerUUID
     * @return Location<World> of player's island
     */
    /*
     * public Location<World> getPlayerIsland(final UUID playerUUID) {
     * if (playerCache.containsKey(playerUUID)) {
            ...
            for (final File f : files) {
                ...
                if (f.getName().endsWith(".yml")) {
                    try {
                        ...
                        plugin.getLogger().error("Problem with " + f.getName());
                    }
                }
            }
        }
    ...
    /**
     * Sets the home Location<World> for the player
     * @param playerUUID
     * @param location
     * @param number - 1 is default. Can be any number.
     */
    public void setHomeLocation(UUID playerUUID, Location<World> location, int number) {
        addPlayer(playerUUID);
        playerCache.get(playerUUID).setHomeLocation(location,number);
    }

    /**
     * Set the default home Location<World> for player
     * @param playerUUID
     * @param location
     */
    public void setHomeLocation(UUID playerUUID, Location<World> location) {
        addPlayer(playerUUID);
     ...
     * @param playerUUID
     * @param number 
     * @return Home Location<World> or null if none
     */
    public Location<World> getHomeLocation(UUID playerUUID, int number) {
        addPlayer(playerUUID);
        return playerCache.get(playerUUID).getHomeLocation(number);
    }

    /**
     * Gets the default home Location<World> for player
     * @param playerUUID
     * @return Home Location<World> or null if none
     */
    public Location<World> getHomeLocation(UUID playerUUID) {
        addPlayer(playerUUID);
     ...
     */
    public HashMap<Integer, Location<World>> getHomeLocations(UUID playerUUID) {
        addPlayer(playerUUID);
     ...
     * Returns an island Location<World> OR a team island location
     * 
     * @param playerUUID
     * @return Location<World> of player's island
     */
    public Location<World> getIslandLocation(UUID playerUUID) {
        addPlayer(playerUUID);
        return playerCache.get(playerUUID).getIslandLocation();
    }
    ...
    public void setIslandLocation(UUID playerUUID, Location<World> islandLocation) {
        addPlayer(playerUUID);
    ...
    public void setTeamIslandLocation(UUID playerUUID, Location<World> islandLocation) {
        addPlayer(playerUUID);
    ...
    public void setJoinTeam(UUID playerUUID, UUID teamLeader, Location<World> islandLocation) {
        addPlayer(playerUUID);
     ...
     * islandLevel, teamIslandLocation, islandLocation<World> and members array
     */
    ...
    public UUID getUUID(String string, boolean adminCheck) {
        ...
        // Try the server
        if (adminCheck && plugin.getServer().getPlayer(string).isPresent()) {
            return plugin.getServer().getPlayer(string).get().getUniqueId();
        }
        return null;
    }
    ...
    public Location<World> getTeamIslandLocation(UUID playerUUID) {
        addPlayer(playerUUID);
     ...
     */
    public UUID getPlayerFromIslandLocation(Location<World> loc) {
        if (loc == null)
            return null;
        // Look in the grid
     ...
     */
    public long getInviteCoolDownTime(UUID playerUUID, Location<World> location) {
        addPlayer(playerUUID);
        return playerCache.get(playerUUID).getInviteCoolDownTime(location);
    }

    /**
     * Starts the timer for the player for this Location<World> before which they can
     * be invited
     ...
     * @param location
     */
    public void startInviteCoolDownTimer(UUID playerUUID, Location<World> location) {
        addPlayer(playerUUID);
    ...
    public boolean isBanned(UUID playerUUID, UUID targetUUID) {
        ...
        if (target.isPresent() && target.get().hasPermission(Settings.PERMPREFIX + "admin.noban")) {
            return false;
        }
        if (playerCache.get(playerUUID).hasIsland()) {
            // Player has island
