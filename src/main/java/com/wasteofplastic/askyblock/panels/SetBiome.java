...
public class SetBiome {
	...
	public SetBiome(final ASkyBlock plugin, final Island island, final BiomeType biomeType) {
		final World world = island.getCenter().getExtent();
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				inProgress = true;
				int count = 0;
				// plugin.getLogger().info("DEBUG: Restart xDone = " + xDone + "
				// zDone = " + zDone);
				// plugin.getLogger().info("DEBUG: max x = " + (island.getMinX()
				...
				while (xDone < (island.getMinX() + island.getIslandDistance())) {
					while (zDone < (island.getMinZ() + island.getIslandDistance())) {
						...
						// plugin.getLogger().info("DEBUG: xDone = " + xDone + "
						// zDone = " + zDone);
						if (count++ > SPEED) {
							// plugin.getLogger().info("DEBUG: set " + SPEED + "
							// blocks");
							inProgress = false;
							return;
						}
				...
				// plugin.getLogger().info("DEBUG: END xDone = " + xDone + "
				// zDone = " + zDone);
				this.cancel();
			}
		...
		// If the biome is dry, then we need to remove the water, ice, snow,
		// etc.
		switch (biomeType) {
		case MESA:
		case DESERT:
		...
		case HELL:
			...
			// plugin.getLogger().info("DEBUG: size of chunk ss = " +
			// chunkSnapshot.size());
			...
			plugin.getServer().getScheduler().runTaskAsynchronously(plugin, new Runnable() {
				...
				public void run() {
							...
							for (int z = 0; z < 16; z++) {
								...
								for (int yy = world.getMaxHeight() - 1; yy >= Settings.sea_level; yy--) {
									...
									if (type == Material.ICE.getId() || type == Material.SNOW.getId()
											|| type == Material.SNOW_BLOCK.getId() || type == Material.WATER.getId()
											|| type == Material.STATIONARY_WATER.getId()) {
										// System.out.println("DEBUG: offending
										// block found " +
										// Material.getMaterial(type) + " @ " +
										// (chunk.getX()*16 + x) + " " + yy + "
										// " + (chunk.getZ()*16 + z));
										blocksToRemove.put(new Vector(chunk.getX() * 16 + x, yy, chunk.getZ() * 16 + z),
												type);
									} else if (type != Material.AIR.getId()) {
										// Hit a non-offending block so break
										// and store this column of vectors
										break;
									}
								}
							}
					...
					if (!blocksToRemove.isEmpty()) {
						// plugin.getLogger().info("DEBUG: There are blocks to
						// remove " + blocksToRemove.size());
						...
						plugin.getServer().getScheduler().runTask(plugin, new Runnable() {
							...
							@Override
							public void run() {
								// plugin.getLogger().info("DEBUG: Running sync
								// task");
								for (Entry<Vector, Integer> entry : blocks.entrySet()) {
									if (entry.getValue() == Material.WATER.getId()
											|| entry.getValue() == Material.STATIONARY_WATER.getId()) {
