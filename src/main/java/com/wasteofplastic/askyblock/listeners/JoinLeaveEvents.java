...
public class JoinLeaveEvents {
	...
	public void onPlayerJoin(final ClientConnectionEvent.Join event) {
		...
		if (player.hasPermission(Settings.PERMPREFIX + "island.lang")) {
			if (DEBUG)
			...
			if (Sponge.getAssetManager().getAsset("locale/" + language + ".yml").isPresent()) {
				if (DEBUG)
		...
		if (players == null) {
			plugin.getLogger().error("players is NULL");
		}
		...
		if (messages != null) {
			...
			plugin.getServer().getScheduler().runTaskLater(plugin, new Runnable() {
				@Override
				public void run() {
					player.sendMessage(Text.of(TextColors.AQUA, plugin.myLocale(playerUUID).newsHeadline));
					int i = 1;
					for (String message : messages) {
						player.sendMessage(Text.of(i++ + ": " + message));
					}
					// Clear the messages
		...
		// Rationalize any out-of-sync issues, which may occur if the server
		// wasn't shutdown properly, etc.
		// Leader or solo
		if (DEBUG)
		...
		if (players.hasIsland(playerUUID)) {
			...
			leader = players.getTeamLeader(playerUUID);
			if (leader == null) {
				plugin.getLogger().error("Player " + player.getName() + " is in a team but leader's UUID is missing.");
			}
		}
		...
		if (loc != null && leader != null) {
			...
			if (island == null) {
				...
				if (islandByOwner == null) {
					// No previous ownership, so just create the new island in
					// the grid
					if (plugin.getGrid().onGrid(loc)) {
						plugin.getGrid().addIsland(loc.getBlockX(), loc.getBlockZ(), leader);
					} else {
						plugin.getLogger().error(player.getName() + " joined and has an island at " + loc
								+ " but those coords are NOT on the grid! Use admin register commands to correct!");
					}
				} else {
					...
					plugin.getLogger().warn(player.getName()
							+ " login: mismatch - player.yml and islands.yml are out of sync. Fixing...");
				...
				// plugin.getLogger().info("DEBUG: getIslandLoc is not null -
				// island exists");
				// See if this owner is tagged as having an island elsewhere
				Island islandByOwner = plugin.getGrid().getIsland(leader);
				if (islandByOwner == null) {
					plugin.getLogger().warn(
							player.getName() + " login: has island, but islands.yml says it is unowned, correcting...");
					...
					if (!islandByOwner.equals(island)) {
						...
						plugin.getLogger().warn(player.getName()
								+ " login: mismatch - islands.yml and player.yml are out of sync. Fixing...");
						// We have a mismatch - correct in favor of the player
						// info
						...
						if (island.getOwner().equals(player.getUniqueId())) {
							if (DEBUG) {
								plugin.getLogger()
										.info("DEBUG: This player owns the island and island protection size is "
							...
							if (range % 2 != 0) {
								range--;
								if (DEBUG)
									plugin.getLogger().warn("Login range setting: Protection range must be even, using "
											+ range + " for " + player.getName());
							}
							if (range > (Settings.islandDistance - 16)
									&& !plugin.getConfig().getBoolean("island.overridelimit", false)) {
								range = Settings.islandDistance - 16;
								if (DEBUG)
									plugin.getLogger()
											.warn("Login range setting: Island protection range must be "
													+ (Settings.islandDistance - 16)
													+ " or less, (island range -16). Setting to: " + range);
							}
							if (range > islandByOwner.getProtectionSize()) {
								plugin.getLogger()
										.info("Login range setting: Island protection range increased from "
		...
		// Set the player's name (it may have changed), but only if it isn't
		// empty
		if (!player.getName().isEmpty()) {
			...
			plugin.getLogger().warn("Player that just logged in has no name! " + playerUUID.toString());
		}
		if (DEBUG)
	...
	public void onPlayerQuit(final ClientConnectionEvent.Disconnect event) {
		// Remove from TopTen if the player has the permission
		if (!event.getTargetEntity().hasPermission(Settings.PERMPREFIX + "intopten")) {
			TopTen.topTenRemoveEntry(event.getTargetEntity().getUniqueId());
		}
		// Remove from coop list
		if (!Settings.persistantCoops) {
			CoopPlay.getInstance().clearMyCoops(event.getTargetEntity());
			CoopPlay.getInstance().clearMyInvitedCoops(event.getTargetEntity());
		}
		plugin.getChatListener().unSetPlayer(event.getTargetEntity().getUniqueId());
		...
		players.removeOnlinePlayer(event.getTargetEntity().getUniqueId());
	}
