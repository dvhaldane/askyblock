 ...
 * asynchronously (and usually it is when a player chats) it cannot access any
 * HashMaps or Bukkit APIs without running the risk of a clash with another
 * thread or the main thread. As such two things are done: 1. To handle the
 * level substitution, a thread-safe hashmap of players and levels is stored and
 * updated in this class. 2. To handle team chat, a thread-safe hashmap is used
 * to store whether team chat is on for a player or not and if it is, the team
 * chat itself is queued to run on the next server tick, i.e., in the main
 * thread This all ensures it's thread-safe.
 * 
 * @author tastybento
 *
 */
public class ChatListener {
	...
	public ChatListener(ASkyBlock plugin) {
		...
		for (Player player : plugin.getServer().getOnlinePlayers()) {
			playerLevels.put(player.getUniqueId(),
					String.valueOf(plugin.getPlayers().getIslandLevel(player.getUniqueId())));
	...
	public void onChat(final MessageChannelEvent.Chat event) {
		if (DEBUG)
			plugin.getLogger().info("DEBUG: " + event.toString());
		...
		if (player.isPresent()) {
			...
			if (playerLevels.containsKey(p.getUniqueId())) {
				level = playerLevels.get(p.getUniqueId());
			}
			...
			if (Settings.teamChat && teamChatUsers.containsKey(event.getPlayer().getUniqueId())) {
				...
				// Queue the sync task because you cannot use HashMaps
				// asynchronously. Delaying to the next tick
	...
	private void teamChat(final AsyncPlayerChatEvent event, String message) {
		...
		if (plugin.getPlayers().inTeam(playerUUID)) {
			...
			message = plugin.myLocale(playerUUID).teamChatPrefix.replace(Settings.chatIslandPlayer,
			...
			if (onLine) {
				for (Player onlinePlayer : plugin.getServer().getOnlinePlayers()) {
					if (spies.contains(onlinePlayer.getUniqueId())
							&& onlinePlayer.hasPermission(Settings.PERMPREFIX + "mod.spy")) {
	...
	public void setPlayerLevel(UUID playerUUID, int level) {
		// plugin.getLogger().info("DEBUG: putting " + playerUUID.toString() + "
		// Level " + level);
	...
	public void setPlayerChallengeLevel(Player player) {
		// plugin.getLogger().info("DEBUG: setting player's challenge level to "
		// + plugin.getChallenges().getChallengeLevel(player));
		playerChallengeLevels.put(player.getUniqueId(), plugin.getChallenges().getChallengeLevel(player));
	}
