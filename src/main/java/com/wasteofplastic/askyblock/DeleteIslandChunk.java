...
public class DeleteIslandChunk {
	...
	// private HashMap<Location, Material> blocksToClear = new
	...
	public DeleteIslandChunk(final ASkyBlock plugin, final Island island) {
		...
		final Chunk minChunk = world.getChunk(minx, 0, minz).get();
		final Chunk maxChunk = world.getChunk(maxx, 0, maxz).get();
		...
		for (int x = minChunk.getX(); x <= maxChunk.getX(); x++) {
			for (int z = minChunk.getZ(); z <= maxChunk.getZ(); z++) {
				...
				if (world.getChunkAt(x, z).getBlock(0, 0, 0).getX() < minxX) {
					// plugin.getLogger().info("DEBUG: min x coord is less than
					// absolute min! "
					// + minxX);
					regen = false;
				}
				if (world.getChunkAt(x, z).getBlock(0, 0, 0).getZ() < minzZ) {
					// plugin.getLogger().info("DEBUG: min z coord is less than
					// absolute min! "
					// + minzZ);
					regen = false;
				}
				if (world.getChunkAt(x, z).getBlock(15, 0, 15).getX() > maxxX) {
					// plugin.getLogger().info("DEBUG: max x coord is more than
					// absolute max! "
					// + maxxX);
					regen = false;
				}
				if (world.getChunkAt(x, z).getBlock(15, 0, 15).getZ() > maxzZ) {
					// plugin.getLogger().info("DEBUG: max z coord in chunk is
					// more than absolute max! "
					// + maxzZ);
					regen = false;
				}
				if (regen) {
		...
		if (!chunksToClear.isEmpty()) {
			...
			plugin.getLogger()
					.info("Island delete: There are " + chunksToClear.size() + " chunks that need to be cleared up.");
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					while (it.hasNext() && count++ < Settings.cleanRate) {
						Pair pair = it.next();
						// plugin.getLogger().info("DEBUG: There are " +
						// chunksToClear.size() + " chunks that need to be
						// cleared up");
						// plugin.getLogger().info("DEBUG: Deleting chunk " +
						...
						for (int x = 0; x < 16; x++) {
							for (int z = 0; z < 16; z++) {
								...
								if (island.inIslandSpace(xCoord, zCoord)) {
									// plugin.getLogger().info(xCoord + "," +
									// zCoord + " is in island space - deleting
									// column");
									// Delete all the blocks here
									for (int y = 0; y < ASkyBlock.getIslandWorld().getBlockMax().getY(); y++) {
										// Overworld
										BlockState b = ASkyBlock.getIslandWorld().getBlock(xCoord, y, zCoord);
										BlockType bt = b.getType();
										BlockType setTo = BlockTypes.AIR;
										// Split depending on below or above
										// water line
										if (y < Settings.sea_level) {
											setTo = BlockTypes.WATER;
										}
										// Grab anything out of containers (do
										// that it is
										// destroyed)
										switch (bt) {
										case CHEST:
										...
										if (Settings.newNether && Settings.createNether
											...
											if (!b.equals(Material.AIR)) {
												setTo = Material.AIR;
												// Grab anything out of
												// containers (do that it is
												// destroyed)
												switch (bt) {
												case CHEST:
												case TRAPPED_CHEST:
