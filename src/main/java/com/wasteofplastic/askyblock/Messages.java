...
public class Messages {
	...
	private CommentedConfigurationNode messageStore;
	...
	public void saveMessages() {
		...
		try {
			...
			// Convert to YAML
			messageStore.getNode("messages").setValue(offlineMessages);
			Util.saveFile(messageStore, "messages.json");
			return;
		} catch (Exception e) {
			e.printStackTrace();
			return;
	...
	public boolean loadMessages() {
		...
		messageStore = Util.loadFile("messages.json");
	...
	public void tellTeam(UUID playerUUID, String message) {
		...
		for (UUID member : teamMembers) {
			// getLogger().info("DEBUG: trying UUID " + member.toString());
			if (!member.equals(playerUUID) && plugin.getServer().getPlayer(member).isPresent()) {
				// Online player
				plugin.getServer().getPlayer(member).get()
						.sendMessage(TextSerializers.FORMATTING_CODE.deserialize(message));
			}
		}
	}
