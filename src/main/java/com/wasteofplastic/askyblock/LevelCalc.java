...
public class LevelCalc extends BukkitRunnable {
    ...
    private CommandSource asker;
    private int islandLevelHandicap;

    public LevelCalc(ASkyBlock plugin, UUID targetPlayer, CommandSource asker) {
     ...
     * @param report
     */
    public LevelCalc(ASkyBlock plugin, UUID targetPlayer, final CommandSource sender, boolean report) {
        this.asker = sender;
        this.plugin = plugin;
    ...
    public void run() {
        ...
        if (counter <= 0) {
            ...
            Optional<Player> player = plugin.getServer().getPlayer(targetPlayer);
            int multiplier = 1;
            if (player.isPresent()) {
                // Get permission multiplier                
                for (PermissionAttachmentInfo perms : player.get().getEffectivePermissions()) {
            ...
            if (asker != null) {
                if (!(asker instanceof Player)) {
                    // Console
                    asker.sendMessage(Text.of(TextColors.GREEN + plugin.myLocale().islandislandLevelis + " " + TextColors.WHITE + plugin.getPlayers().getIslandLevel(targetPlayer)));
                    if (Settings.deathpenalty != 0) {
                        asker.sendMessage(Text.of(TextColors.GREEN + "(" + String.valueOf(plugin.getPlayers().getDeaths(targetPlayer)) + " " + plugin.myLocale().deaths + ")"));
                    }
                    asker.sendMessage(TextColors.GREEN + "Level report only available with fast-level calculation option");
                } else {
                    ...
                    if (plugin.getPlayers().getIslandLevel(targetPlayer) > oldLevel) {
                        ...
                        plugin.getMessages().tellOfflineTeam(targetPlayer, TextColors.GREEN + plugin.myLocale(targetPlayer).islandislandLevelis + " " + TextColors.WHITE
                    ...
                    if (asker instanceof Player && ((Player)asker).isOnline()) {
                        ...
                        String message = TextColors.GREEN + plugin.myLocale(((Player)asker).getUniqueId()).islandislandLevelis + " " + TextColors.WHITE + plugin.getPlayers().getIslandLevel(targetPlayer);
                        if (Settings.deathpenalty != 0) {
        ...
        for (int y = top; y >= bottom; y--) {
            ...
            for (int x = px - r; x <= px + r; x++) {
                for (int z = pz - r; z <= pz + r; z++) {
                    BlockType blockType = l.getExtent().getBlock(x, y, z).getType();
                    byte data = l.getExtent().getBlock(x, y, z).getData();
